    ...
    render: function(elementId, width, height, toScale) {
    	...
    	if(typeof(toScale)==='undefined') toScale = true;
			...
			if(toScale) {
				...
				//sourceWidth = 1191;
				//sourceHeight = 842;
				p1 = {name: 'borg', path: polygonToPath("533.438,174.333 444.271,159.833 442.771,167.083 441.271,236.667 442.104,243 533.438,234.167 533.438,229.5 532.104,219.167 531.701,201.417 532.457,184.835")};
				p2 = {name: 'pingala', path: polygonToPath("533.438,237.667 497.188,241.167 504.75,272.292 537.5,264.167 534.812,249")};
				p3 = {name: 'manning', path: polygonToPath("538.187,268.913 506.533,276.992 513.708,306.542 546.939,294.004 542.167,281.5")};
				p4 = {name: 'forum', path: polygonToPath("429.688,252.625 471.625,249.417 488.5,318.167 463.188,327.5 460.938,323.5 448.188,327.75 450.25,332.833 354,370.333 351.938,364.25 344.938,344.5 338.688,324.25 334.188,306.75 331.188,286.25 327.188,265.75 325.438,247.5 322.438,247.5 321.062,228.125 404.015,225.167 406.084,256.412 410.312,256 408.188,222.25 408.5,177.167 388,174.917 389.438,161.25 434.188,167 433.438,237.25 428.555,237.813")};
				...
				p12 = {name: 'lovelace', path: polygonToPath("575,569.5 559.271,601 576.938,610.333 595.938,619 595.938,620 602.604,623 616,588.167")};
				p13 = {name: 'snowden', path: polygonToPath("624.448,583.75 607.552,625.333 611.938,626.5 612.604,626.5 631.771,632.333 652.438,638.5 652.438,639.833 659.438,641.333 670.333,598.5")};
				p14 = {name: 'turing', path: polygonToPath("682.5,574.667 755,585.526 752.667,658.359 729.913,657.179 729.912,653.811 711.604,651.917 711.738,650.379 669.604,643.333 669.604,644.333 665.667,642.667")};
				...
				p17 = {name: 'icaza', path: polygonToPath("766.594,491.086 765,532.831 809.188,532.667 816.812,531.542 810.188,490.625")};
				p18 = {name: 'hopper', path: polygonToPath("809.938,485 803.562,445.75 793.438,446.875 776.562,448 759.812,447.625 749.438,447.125 744.312,446.625 739.812,486.5")};
			...
			// var paper = Raphael(elementId, width, height);
				...
				makeRoom('forum',[0,height*0.4,width*0.25,height*0.6]);
				makeRoom('borg',[0,height*0.03,width*0.11,height*0.37]);
				makeRoom('pingala',[width*0.11,height*0.04,width*0.07,height*0.26]);
				makeRoom('manning',[width*0.18,height*0.05,width*0.07,height*0.25]);
				makeRoom('hopper',[width*0.8,height*0.05,width*0.2,height*0.33]);
				makeRoom('icaza',[width*0.85,height*0.38,width*0.15,height*0.27]);
				makeRoom('storeRoom',[width*0.9,height*0.79,width*0.1,height*0.21]);
				makeRoom('turing',[width*0.77,height*0.79,width*0.13,height*0.21]);
				makeRoom('snowden',[width*0.69,height*0.84,width*0.08,height*0.16]);
				makeRoom('lovelace',[width*0.61,height*0.86,width*0.08,height*0.14]);
